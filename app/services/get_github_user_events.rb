  ...
  def self.call(current_user, page_number = 1)
    ...
    begin
      ...
      request.each do |event|
        ...
        # Create a new EventTimeline and if the new GithubEvent is created successfully move to the next event, else delete the new timeline.
        new_event_timeline = EventTimeline.create(user_id: current_user.id, created_at: event["created_at"])
