class SortActivityFeed
  def self.call(current_user, page_number = 1)
    ...
    if current_user.passed_four_stars.present?
      user_activites += [Activity.new("passed_four_stars", { average_rating: current_user.average_rating.to_f.round(1) }, current_user.passed_four_stars)]
    end
    ...
    begin
      ...
      user_activites += github_events.reject { |s| s[:timestamp] == nil }.map do |github_event|
        Activity.new("github", { event_type: github_event[:type], repository: github_event[:repository], branch: github_event[:brach], pull_request_number: github_event[:pull_request_number], commit_count: github_event[:commit_count] }, github_event[:timestamp].to_datetime)
      end
    rescue => error
      puts error
    end
